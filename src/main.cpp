...
#include "Creature/Parts/cBrain.hpp"
#include "Creature/Parts/cNeuron.hpp"
#include "Utils/cRandomPool.hpp"
#include "Creature/cSpotCreature.hpp"
...
int evaluate(Creature::cSpotCreature & spotty, cRandomPool & pool)
{
    double food;
    int score = 0;
...
std::vector<Creature::cSpotCreature> initial_generation(cRandomPool & pool)
{
  std::vector<Creature::cSpotCreature> spots;
    for (int i = 0; i < 100; i++)
    {
      Creature::cSpotCreature spotty(pool, 5, pool.generate_ID(0));
        spots.push_back(spotty);
    }
    return spots;
}

std::vector<Creature::cSpotCreature> create_generation(Creature::cSpotCreature & father, cRandomPool & pool)
{
  std::vector<Creature::cSpotCreature> spots;
    for (int i = 0; i < 100; i++)
    {
      Creature::cSpotCreature spotty(father, pool);
        spots.push_back(spotty);
    }
    return spots;
}
...
{
  cRandomPool random(1);

  std::vector<Creature::cSpotCreature> spots;
    spots = initial_generation(random);

    Creature::cSpotCreature best(random, 4, 0);
